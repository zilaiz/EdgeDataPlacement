...
public class ECGreedyDegreeModel {
    ...
    public ECGreedyDegreeModel(int serversNumber, int [][]distancematrix, int hops) //构造函数
    {
    ...
    public void runECGreedyDegreeCost() throws ClassNotFoundException, IOException
    { // 生成放置方案
        ...
        for (int m = 1; m <= mindeg; ++m)
        {
            ...
            {
                int MostDegreeServer = selectServerwithMostDegrees(); //VoteHighest为票数最高的服务器
                SelectedServerList.add(MostDegreeServer);
                mDegrees.remove(MostDegreeServer);  //如果某节点已经被选择，则在mDegrees中删除
                for (int c = 0; c < mServersNumber; ++c)
                {  //对VoteHighestServer可访问节点的mDataPacketsNeed值减1
                    if(mAccessMatrix[MostDegreeServer][c] == 1)
                    {
                        int v = mDataPacketsNeed.get(c);
                        if(v > 0){
                            mDataPacketsNeed.put(c,v-1); 
                        }
                        if(mDegrees.containsKey(c) && c != MostDegreeServer)
                        {
                            int d = mDegrees.get(c);
                            if(d > 0)
                            {
                                mDegrees.put(c,d-1); //更新被选节点周围非数据节点的度数
                            }
                        }
                    }
                }
        ...
        // System.out.println("\nBestOfGreedyDegree:" + best_pn + ":" + MinCostServerList + " cost:" + min_cost);
        mCost = min_cost;
